...
class TFPublish():
    def __init__(self):
        ...
        # 设置父坐标系
        static_transformStamped.header.frame_id = "base_link"  # A
        # 设置子坐标系
        static_transformStamped.child_frame_id = "head_link" # B
        # 设置父坐标系到子坐标系的平移变换  # X  => B = XA
    ...
    def pose_cb(self, msg):
        ...
        Args:
            msg (_type_): Marker发布的姿态信息, 数据是在word下的位姿
        """
        # 创建TransformStamped对象dynamic_tf_1: world — base_link
        dynamic_tf_1 = TransformStamped()
        dynamic_tf_1.header.stamp = rospy.Time.now()
        dynamic_tf_1.header.frame_id = "world"  # A
        dynamic_tf_1.child_frame_id = "base_link"  # B
        # 平移变换随Marker变化，base_link始终位于Marker下方  # X => B = XA
        ...
        q1 = transformations.quaternion_from_euler(0, 0, 0)  # 欧拉角转四元数
        ...
        dynamic_tf_2.transform.translation.z = 0.08  # camera_link 相对于 head_link的平移是固定的
        pitch = math.atan((msg.pose.position.z - 0.08 - 0.3)/0.1) -3.1415926/2
        q2 = transformations.quaternion_from_euler(0, pitch, 0)  # camera_link的俯仰角不是固定的
