...
ARTrackAndPick::ARTrackAndPick(ros::NodeHandle &nh):nh_(nh),table_id_("table"){
  ...
  // 创建服务/vision_demo_pickup的服务端
  pick_place_srv_ = nh_.advertiseService(std::string("/vision_demo_pickup"), &ARTrackAndPick::callPickPlace, this);
  ROS_INFO("Pick and Place demo is ready. You can call the service /vision_demo_pickup to test... ");
...
// /vision_demo_pickup服务处理函数
bool ARTrackAndPick::callPickPlace(vision_demo::CallPickPlaceDemo::Request &req,
                                   vision_demo::CallPickPlaceDemo::Response &res){
