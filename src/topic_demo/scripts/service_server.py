...
import platform
import rospy
from topic_demo.srv import SetTargetDetec, SetTargetDetecResponse
# 服务的回调处理函数,req为服务的请求
def target_detection_handle(req):
    # 打印输出req.name
    rospy.loginfo('Python version is ' + platform.python_version())
...
def server():
    ...
    rospy.Service('target_detection', SetTargetDetec, target_detection_handle)
